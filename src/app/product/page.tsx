  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
          <div className="absolute inset-0 bg-[linear-gradient(rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.03)_1px,transparent_1px)] bg-[size:50px_50px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,#000_70%,transparent_100%)]" />
      </div>
      ...
      <div className="relative z-10 pt-40 pb-24 px-6 md:px-12 lg:px-24 max-w-[1400px] mx-auto">
        ...
        >
          <div className="inline-block px-4 py-2 mb-6 rounded-full bg-slate-900 text-white text-[10px] font-bold tracking-[0.3em] shadow-lg">
            SYSTEM ARCHITECTURE
          </div>
          <h1 className="text-5xl md:text-8xl font-black tracking-tighter mb-8 leading-[0.9] text-slate-900">
            The Learning Loop
          </h1>
          <p className="text-xl md:text-2xl leading-relaxed font-light text-slate-500 max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16 relative">
          {/* Connecting Line (Desktop) - Light Mode Color */}
          <div className="hidden md:block absolute top-[10%] bottom-[10%] left-1/2 w-px bg-slate-200 -translate-x-1/2" />
          ...
          {steps.map((step, i) => (
            ...
            >
              {/* Dot on line */}
              <div 
                className={`hidden md:flex absolute top-12 w-4 h-4 rounded-full border-4 border-white shadow-md items-center justify-center z-10
                  ${i % 2 === 0 ? "right-[-8px] translate-x-1/2" : "left-[-8px] -translate-x-1/2"}`}
                style={{ backgroundColor: step.color }}
              />

              <div className="group relative p-8 md:p-12 rounded-[2rem] bg-white border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] transition-all duration-500 overflow-hidden">
                ...
                <div className={`relative z-10 mb-8 flex items-center gap-4 ${i % 2 === 0 ? "md:flex-row-reverse" : "md:flex-row"}`}>
                  <div 
                    ...
                    style={{ backgroundColor: step.color, boxShadow: `0 4px 15px ${step.color}60` }}
                  >
                    <step.icon size={32} strokeWidth={1.5} />
                  </div>
                  <div className="flex-1">
                    <div className="text-sm font-bold tracking-widest text-slate-400 mb-1">{step.id}</div>
                    <div className="text-xs font-mono font-bold tracking-wider uppercase" style={{ color: step.color }}>{step.role}</div>
                  </div>
                </div>

                <h3 className="relative z-10 text-2xl md:text-3xl font-bold mb-2 tracking-tight text-slate-900">{step.title}</h3>
                <h4 className="relative z-10 text-lg font-bold text-slate-400 mb-6">{step.subtitle}</h4>
                
                <p className="relative z-10 text-base leading-loose text-slate-600 font-medium mb-6">
                  {step.description}
                </p>
                  ...
                  <div className="relative z-10 space-y-3 pt-6 border-t border-slate-100">
                    {step.details.map((detail, idx) => (
                      <div key={idx} className={`text-sm ${i % 2 === 0 ? "md:flex-row-reverse" : ""}`}>
                        ...
                        <span className="block text-slate-700 font-medium">{detail.value}</span>
                      </div>
                    ))}
                  </div>
                )}
