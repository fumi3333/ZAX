/**
 * ユーザーデータ確認スクリプト (TypeScriptエイリアスなし版)
 * 特定ユーザー（s2527084）のデータをDB上で確認する
 */
require('dotenv').config({ path: require('path').join(__dirname, '..', '.env') });

import { PrismaClient } from '@prisma/client';

// DATABASE_URLをそのまま使う（pgbouncer等のVercel特有のオプション不要）
...
async function main() {
  ...
  for (const u of users) {
    ...
    console.log(`${tag} ${u.email.substring(0, 40).padEnd(40)} | 診断${u._count.diagnostics}件 | ベクトル${u._count.vectors}件 | ${new Date(u.createdAt).toLocaleString('ja-JP')}`);
  }
